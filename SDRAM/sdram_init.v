module sdram_init #(
    parameter INIT_T_POWER  = 15'd20_000,
    parameter INIT_TRP_CLK  = 2,
    parameter INIT_TRF_CLK  = 7,
    parameter INIT_TRF_CNT  = 8,
    parameter INIT_TMRD_CLK = 3,
    ...
    reg [14:0] init_cnt_200us;
    reg        init_wait_end;
    reg [ 7:0] init_current_state;
    reg [ 7:0] init_next_state;
    reg [ 3:0] init_cnt_state;
    reg [ 3:0] init_cnt_ref;

    always @(*) begin  //次态跳转
        case (init_current_state)
            INIT_IDLE: begin
                if (init_wait_end) begin
            ...
            INIT_TRP: begin
                if (init_cnt_state == INIT_TRP_CLK) begin
            ...
            INIT_TRF: begin
                if (init_cnt_ref == INIT_TRF_CNT && init_cnt_state == INIT_TRF_CLK) begin
                    init_next_state = INIT_MRS;
                end else if (init_cnt_state == INIT_TRF_CLK) begin
                    init_next_state = INIT_AR;
                end else begin
                    init_next_state = INIT_TRF;
                end
            ...
            INIT_TMRD: begin
                if (init_cnt_state == INIT_TMRD_CLK) begin
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin  //自刷新次数计数器
        if (!sys_rst_n) begin
            ...
            init_cnt_ref <= init_cnt_ref + 4'd1;
        end else begin
            init_cnt_ref <= init_cnt_ref;
        end
    end

    always @(posedge sys_clk or negedge sys_rst_n) begin  //上电等待时间计数器
        if (!sys_rst_n) begin
            init_cnt_200us <= 15'd0;
        end else if (init_cnt_200us < INIT_T_POWER) begin
            init_cnt_200us <= init_cnt_200us + 15'd1;
        end else begin
            init_cnt_200us <= init_cnt_200us;
        end
    end
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin  //等待完成标志信号
        if (!sys_rst_n) begin
            init_wait_end <= 1'd0;
        end else if (init_cnt_200us == INIT_T_POWER - 2) begin
            init_wait_end <= 1'd1;
        end else begin
            init_wait_end <= 1'd0;
        end
